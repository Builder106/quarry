...
const DRILLING_LOG = [
  {
    step: '01',
    depth: '100 m',
    title: 'WebSocket pending-tx',
    body: 'Subscribe to mempool over WS via viem.',
  },
  {
    step: '02',
    depth: '140 m',
    title: 'Router filter',
    body: 'Uniswap V2 + Sushiswap only. Everything else dropped.',
  },
  {
    step: '03',
    depth: '200 m',
    title: 'Calldata decode',
    body: 'Four router methods, discriminated union — no dynamic dispatch.',
  },
  {
    step: '04',
    depth: '280 m',
    title: 'Multicall reserves',
    body: 'Multicall3 reads both pair reserves in a single RPC round-trip.',
  },
  {
    step: '05',
    depth: '360 m',
    title: 'Score the back-run',
    body: 'Apply victim → solve x* = (r·√K − R₁ᵢₙ·R₂ᵢₙ) / (r·(R₂ᵢₙ + r·R₁ₒᵤₜ)).',
  },
  {
    step: '06',
    depth: '460 m',
    title: 'Gate against gas',
    body: 'Refuse if profit ≤ flashloan premium + bundle gas cost.',
  },
  {
    step: '07',
    depth: '580 m',
    title: 'Pack 220-byte calldata',
    body: 'No ABI offsets. Tight byte string designed for calldataload.',
  },
  {
    step: '08',
    depth: '720 m',
    title: 'Sign + bundle',
    body: 'EIP-1559 envelope → Aave V3 flashLoanSimple → Yul executor.',
  },
  {
    step: '09',
    depth: '870 m',
    title: 'Yul executes',
    body: 'Borrow → swap → swap → assert → approve → return profit.',
  },
] as const;
...
function QuarryMark() {
  return (
    <svg aria-hidden viewBox="0 0 64 64" className="h-7 w-7" fill="none">
      ...
      <header className="border-b border-[var(--color-vein-line)] bg-[var(--color-shaft)]/80 backdrop-blur sticky top-0 z-40">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4 lg:px-12">
          ...
          <nav className="flex items-center gap-6 text-sm">
            <a href="#log" className="text-[var(--color-dust)] hover:text-[var(--color-ore)]">
              Drilling log
            </a>
            <a href="#demo" className="text-[var(--color-dust)] hover:text-[var(--color-ore)]">
              Field demo
            </a>
            <a
              href={GITHUB_URL}
              className="inline-flex items-center gap-1.5 font-mono text-xs tracking-[0.15em] uppercase text-[var(--color-ore)] hover:text-[var(--color-pearl)]"
            >
              <GithubIcon className="h-3.5 w-3.5 shrink-0 fill-current" />
              <span>Source ↗</span>
            </a>
          </nav>
        </div>
      ...
      <main className="relative mx-auto max-w-6xl px-6 lg:px-12 lg:pr-24 pb-32">
          ...
          <div className="grid lg:grid-cols-12 gap-10">
            <div className="lg:col-span-8">
              ...
              <p className="mt-10 text-lg leading-relaxed text-[var(--color-dust)] max-w-2xl">
                A bare-metal MEV arbitrage engine. A TypeScript scanner watches Ethereum&apos;s
                public mempool for swaps about to land on a Uniswap-V2-shaped DEX; for each
                candidate it back-computes the price the victim will leave behind, runs a
                closed-form optimal-input solver, and — if the round-trip profit clears fees and gas
                — signs an EIP-1559 transaction that borrows from Aave V3 and routes through a{' '}
                <span className="text-[var(--color-ore)] font-medium">188-byte Yul executor</span>.
              </p>

              <p className="mt-5 text-[var(--color-dust)] max-w-2xl">
                Every opcode shaved off the on-chain leg widens the marginal profit envelope.
                That&apos;s the engineering thesis.
              </p>
              ...
              <div className="mt-12 max-w-2xl field-note text-sm">
                <span className="font-medium not-italic text-[var(--color-pearl)]">Scope.</span>{' '}
                Quarry targets cross-DEX arbitrage — closing price gaps the market would close
                anyway. The kind of MEV broadly considered net-positive for on-chain price
                efficiency. Predatory strategies (sandwiches, JIT against retail) are out of scope.
              </div>
            </div>
            ...
            <aside className="lg:col-span-4">
              <div className="lg:sticky lg:top-32 border border-[var(--color-vein-line)] bg-[var(--color-rock)] p-6 font-mono text-xs">
                ...
                <div className="text-[var(--color-shadow)] leading-relaxed">
                  Filed under field operations. Sample integrity verified against forked mainnet
                  (anvil HEAD). Cross-referenced with JOURNAL.md.
                </div>
              </div>
            </aside>
          </div>
        ...
        <section className="mt-32">
          ...
          <p className="mt-4 text-[var(--color-dust)] max-w-2xl">
            Four headline figures from the working operation. Pulled from the repository&apos;s test
            ledger; reproducible against the published anvil fork harness.
          </p>
            ...
            {SAMPLES.map((s) => (
              <article key={s.label} className="tag">
                ...
                <div className="mt-6 flex items-baseline gap-2">
                  ...
                  <span className="font-mono text-sm text-[var(--color-dust)]">{s.unit}</span>
                </div>
                <div className="mt-3 font-medium text-[var(--color-pearl)]">{s.label}</div>
          ...
          <div className="grid lg:grid-cols-12 gap-10">
            <div className="lg:col-span-5">
              ...
              <p className="mt-4 text-[var(--color-dust)] leading-relaxed">
                Nine deterministic steps from a pending mempool transaction to a signed
                Flashbots-shaped bundle. The off-chain side does all the math; the on-chain Yul
                executor is monolithic — no Solidity, no function dispatcher, no ABI decoding, just{' '}
                <code className="font-mono text-[var(--color-ore)]">calldataload</code> reads
                against a tightly packed 220-byte payload.
              </p>
              <p className="mt-6 text-sm text-[var(--color-shadow)] leading-relaxed">
                Pool 1&apos;s output flows directly to pool 2 — the canonical Uniswap V2 trick,
                saving ~25k gas of intermediate ERC20 transfer. A balance snapshot at entry and exit
                guards the trade: if the arbitrage window closes between detection and inclusion,
                the whole transaction reverts and only the base network fee is burned.
              </p>
            </div>
              ...
              {DRILLING_LOG.map((s) => (
                <li key={s.step} className="borehole">
                  ...
                  <div className="flex items-baseline gap-3 mb-1">
                    ...
                    <h3 className="text-[var(--color-pearl)] font-medium">{s.title}</h3>
                  </div>
                  <p className="text-sm text-[var(--color-dust)] leading-relaxed">{s.body}</p>
                </li>
              ))}
            </ol>
          </div>
        ...
        <section id="demo" className="mt-32 scroll-mt-24">
          ...
          <p className="mt-4 text-[var(--color-dust)] max-w-3xl">
            The full pipeline runs against a local anvil fork. The bot holds zero inventory; Aave V3
            fronts the WETH and gets repaid atomically plus a 5 bp premium inside the same
            transaction.
          </p>
          ...
          <figure className="mt-10">
            ...
            <figcaption className="mt-3 text-sm text-[var(--color-shadow)] font-mono">
              Back-run pipeline scoring a 1 M USDC victim swap, borrowing 5.3 WETH from Aave V3,
              netting 0.557 WETH net of premium.
            </figcaption>
          </figure>
          ...
          <p className="mt-6 text-sm text-[var(--color-shadow)] max-w-3xl">
            The 0.11% drift between net predicted and net realised is exactly the 2 bp safety margin
            baked into the calldata builder for Uniswap V2&apos;s K-invariant integer-arithmetic
            check —{' '}
            <a
        ...
        <section className="mt-32">
          ...
          <p className="mt-4 text-[var(--color-dust)] max-w-3xl">
            Two intentionally independent trees. They communicate via deployed contract address +
            ABI only — never via a shared TS package. Each side has its own toolchain, its own test
            suite, its own gas/perf budget.
          </p>
            ...
            {INVENTORY.map((crate) => (
              ...
              >
                <div className="flex items-baseline justify-between">
                  <div className="font-mono text-sm text-[var(--color-ore)]">{crate.crate}</div>
          ...
          <div className="relative border border-[var(--color-vein-line)] bg-gradient-to-br from-[var(--color-face)] to-[var(--color-rock)] p-10 md:p-16 overflow-hidden">
            ...
            <div className="relative">
              ...
              <p className="mt-6 text-[var(--color-dust)] max-w-2xl leading-relaxed">
                Read the JOURNAL for the incidents — selector bugs, K-invariant boundary drift,
                whale-balance shortages — that shaped the design. Every entry is a sample log.
              </p>
              <div className="mt-10 flex flex-wrap gap-4">
                <a href={GITHUB_URL} className="cta primary">
                  Read the source
        ...
        <footer className="mt-24 pt-8 border-t border-[var(--color-vein-line)]">
          <div className="flex flex-wrap items-center justify-between gap-4 font-mono text-xs tracking-[0.15em] uppercase text-[var(--color-shadow)]">
            <div>
              ...
              {' · '}
              V3 · 2026 ·{' '}
              <a href={`${GITHUB_URL}/blob/main/LICENSE`} className="hover:text-[var(--color-ore)]">
                MIT
              </a>
            </div>
            <div>Surveyed with Next.js · cut on Vercel</div>
          </div>
        </footer>
      </main>
    </>
